...
namespace ScriptableAnimation
{
    ...
    public static class AnimationTools
    {
        public static Animation Repeat(Animation animation, int count)
        {
            return async (token) =>
            {
                await UniTask.WhenAll(Enumerable.Range(0, count).Select(_ => animation.Invoke(token)));
            };
        }


        public static Animation<T> Repeat<T>(Animation<T> animation, int count)
        {
            return async (context, token) =>
            {
                await UniTask.WhenAll(Enumerable.Range(0, count).Select(_ => animation.Invoke(context, token)));
            };
        }
        ...
        public static async UniTask Produce(int time, CancellationToken token)
        {

            //before
            while (token.IsCancellationRequested && CanProduce())
            {
        ...
        {
            var animations = others; // Захватываем массив один раз
            return async token => await UniTask.WhenAll(animations.Select(anim => anim(token)));
        }
        ...
        public static Animation NonParallelSequence(params Animation[] others)
        {
            return async (token) =>
            {
        ...
        public static Animation Sequence(bool parallel = false, params Animation[] others)
        {
            ...
            else @new = AnimationTools.NonParallelSequence(others);
        ...
        {
            var animations = others;
            return async (context, token) => await UniTask.WhenAll(animations.Select((animation) => animation.Invoke(context, token)));
        }
        ...
        public static Animation Sequence(Animation animation, bool parallel = false, params Animation[] others)
        {
            ...
            else @new = async (token) => { await animation.Invoke(token); await AnimationTools.NonParallelSequence(others).Invoke(token); };
